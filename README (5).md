...
A **Hazard Detection and Forwarding Unit** — a purely combinational circuit — monitors the instructions currently sitting in `IF_ID_IR`, `ID_EX_IR`, `EX_MEM_IR`, and `MEM_WB_IR`, and produces three outputs:

| Signal | Width | Purpose |
|---|---|---|
| `ForwardA` | 2 bits | Selects the source for ALU input 1 |
| `ForwardB` | 2 bits | Selects the source for ALU input 2 |
| `load_hazard` | 1 bit | Asserted when the instruction in `ID_EX_IR` is a `LW`, and the instruction currently in `IF_ID_IR` needs the value it's loading |

**Operand forwarding.** At the start of every EX-stage clock edge, `ForwardA`/`ForwardB` are checked to select each ALU operand:

| Value | Source |
|---|---|
| `00` | No forwarding needed — operand taken directly from `ID_EX_A` / `ID_EX_B` |
| `10` | Forwarded from `EX_MEM_ALUOUT` (result of the instruction immediately ahead, currently in EX/MEM) |
| `01` | Forwarded from `MEM_WB_ALUOUT` (result of the instruction two ahead, currently in MEM/WB) |
| `11` | Forwarded from `MEM_WB_LMD` (a loaded value, now available in MEM/WB) |

**Load-use stalling.** Forwarding alone cannot resolve a load-use hazard — a loaded value isn't available until the `LW` instruction completes its MEM stage, one cycle later than an ALU result would be. When `load_hazard` goes high:

- **IF stage:** `PC` holds its current value (`PC = PC`); `IF_ID` pipeline registers hold their existing values (frozen for one cycle)
- **ID stage:** a NOP (bubble) is inserted into `ID_EX`

One cycle later, the loaded value has reached `MEM_WB_LMD`, and normal forwarding (`ForwardA`/`ForwardB = 11`) supplies it correctly to the stalled instruction as it finally enters EX.

### Control Hazard Handling

A **Branch Prediction Unit** takes `EX_MEM_IR` and `EX_MEM_COND` as inputs. If the branch condition is satisfied, it asserts `branch_flag`.

When `branch_flag` goes high:

- **IF stage:** `PC` is redirected to the branch target (`EX_MEM_ALUOUT`)
- **Flush:** a NOP is inserted into `IF_ID`, `ID_EX`, **and** `EX_MEM` pipeline registers in the same cycle

Three stages are flushed (rather than one or two) because the branch condition is only available as a *registered* signal from `EX_MEM_COND` — by the time it's valid, three younger instructions have already been fetched down the wrong path and must all be squashed together.

### Structural Hazard Handling

- Separate instruction and data memories, so instruction fetch (IF) and data access (MEM) never contend for the same memory port
- Register file with 2 read ports and 1 write port, supporting a simultaneous read (ID) and write (WB) every cycle without conflict
